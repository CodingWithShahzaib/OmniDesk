...
export type UseTextStreamOptions = {
  /** Static string, async iterable, or omit when using `getTextStream`. */
  textStream?: string | AsyncIterable<string>;
  /** Fresh iterable per call — avoids consumed generators (e.g. React Strict Mode). */
  getTextStream?: () => AsyncIterable<string>;
  /** Bump when starting a new stream while using `getTextStream`. */
  streamKey?: number;
  speed?: number;
  mode?: Mode;
  onComplete?: () => void;
...
function useTextStream({
  ...
  const startStreaming = useCallback(() => {
    ...
    if (getTextStream) {
      void processAsyncIterable(getTextStream());
      return;
    }
...
export type ResponseStreamProps = {
  ...
  segmentDelay?: number;
  characterChunkSize?: number;
  /** Render accumulated text as Markdown (typewriter mode). */
  markdown?: boolean;
};
